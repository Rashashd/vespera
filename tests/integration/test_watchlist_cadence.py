...
async def test_default_cadence_is_weekly(client, make_client, make_staff_user):
...
async def test_set_cadence_and_sibling_independence(client, make_client, make_staff_user):
    """Two watchlists hold independent cadences (FR-006)."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    a = (
        await client.post(
            f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(cadence="daily")
        )
    ).json()
    ...
    patched = await client.patch(
        f"/clients/{tenant.id}/watchlists/{b['id']}", headers=h, json={"cadence": "monthly"}
    )
    assert patched.status_code == 200 and patched.json()["cadence"] == "monthly"
    # a unchanged
    a_again = await client.get(f"/clients/{tenant.id}/watchlists/{a['id']}", headers=h)
    assert a_again.json()["cadence"] == "daily"


async def test_invalid_cadence_is_422(client, make_client, make_staff_user):
    """An out-of-set cadence is rejected with 422 (FR-006)."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    resp = await client.post(
        f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(cadence="hourly")
    )
