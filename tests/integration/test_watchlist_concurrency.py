...
async def test_concurrent_create_same_name(client, make_client, make_staff_user):
    ...
    results = await asyncio.gather(
        *(
            client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(name))
            for _ in range(8)
        )
    )
    ...
    listing = (await client.get(f"/clients/{tenant.id}/watchlists", headers=h)).json()
    assert sum(1 for w in listing if w["name"] == name) == 1


async def test_concurrent_duplicate_item_add(client, make_client, make_staff_user):
    ...
    wl = (
        await client.post(
            ...
            headers=h,
            json=_payload(f"WL-{uuid.uuid4().hex[:8]}"),
        )
    ).json()
    item = {"item_type": "keyword", "value": "rash"}
    results = await asyncio.gather(
        *(
            client.post(f"/clients/{tenant.id}/watchlists/{wl['id']}/items", headers=h, json=item)
            for _ in range(8)
        )
    )
    codes = sorted(r.status_code for r in results)
    ...
    final = (await client.get(f"/clients/{tenant.id}/watchlists/{wl['id']}", headers=h)).json()
    assert (
