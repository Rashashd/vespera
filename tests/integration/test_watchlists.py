...
async def test_create_with_items(client, make_client, make_staff_user, auth_app):
    ...
    resp = await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload())
...
async def test_create_empty_is_400(client, make_client, make_staff_user):
    """An empty items list is rejected with 400 WATCHLIST_EMPTY (FR-016)."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    resp = await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(items=[]))
    assert resp.status_code == 400
    assert resp.json()["detail"] == "WATCHLIST_EMPTY"


async def test_payload_dedup(client, make_client, make_staff_user):
...
async def test_two_names_then_duplicate_conflict(client, make_client, make_staff_user):
    """Two distinct names succeed; reusing a name in the same client is a 409."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    n1, n2 = _name(), _name()
    assert (
        await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(n1))
    ).status_code == 201
    assert (
        await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(n2))
    ).status_code == 201
    dup = await client.post(
        f"/clients/{tenant.id}/watchlists", headers=h, json=_payload(n1.upper())
    )
    assert dup.status_code == 409
    assert dup.json()["detail"] == "WATCHLIST_NAME_TAKEN"


async def test_list_and_get(client, make_client, make_staff_user):
    ...
    listing = await client.get(f"/clients/{tenant.id}/watchlists", headers=h)
    assert listing.status_code == 200
    assert created["id"] in {w["id"] for w in listing.json()}
    one = await client.get(f"/clients/{tenant.id}/watchlists/{created['id']}", headers=h)
    assert one.status_code == 200 and one.json()["id"] == created["id"]


async def test_rename(client, make_client, make_staff_user, auth_app):
    ...
    resp = await client.patch(
        f"/clients/{tenant.id}/watchlists/{wl['id']}", headers=h, json={"name": new}
    )
...
async def test_rename_to_sibling_name_is_conflict(client, make_client, make_staff_user):
    """Renaming a watchlist onto a sibling's name (case-insensitive) is a 409 (FR-003)."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    a = (await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload())).json()
    b = (await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload())).json()
    resp = await client.patch(
        f"/clients/{tenant.id}/watchlists/{b['id']}",
        headers=h,
        json={"name": a["name"].upper()},
    )
    assert resp.status_code == 409
    assert resp.json()["detail"] == "WATCHLIST_NAME_TAKEN"


async def test_reactivate(client, make_client, make_staff_user):
...
async def test_deactivate_soft_delete(client, make_client, make_staff_user, auth_app):
    ...
    default = await client.get(f"/clients/{tenant.id}/watchlists", headers=h)
    assert wl["id"] not in {w["id"] for w in default.json()}
    with_inactive = await client.get(
        f"/clients/{tenant.id}/watchlists?include_inactive=true", headers=h
    )
...
async def test_idempotent_item_add(client, make_client, make_staff_user, auth_app):
    ...
    new = await client.post(
        f"/clients/{tenant.id}/watchlists/{wl['id']}/items",
        headers=h,
        json={"item_type": "keyword", "value": "rash"},
    )
...
async def test_remove_absent_item_graceful(client, make_client, make_staff_user):
    """Removing an item that isn't there is a graceful 204 no-op (FR-005)."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    wl = (await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload())).json()
    resp = await client.delete(
        f"/clients/{tenant.id}/watchlists/{wl['id']}/items/99999999", headers=h
    )
    assert resp.status_code == 204


async def test_remove_to_empty_is_400(client, make_client, make_staff_user):
    ...
    resp = await client.delete(
        f"/clients/{tenant.id}/watchlists/{wl['id']}/items/{only_item}", headers=h
    )
    assert resp.status_code == 400
    assert resp.json()["detail"] == "WATCHLIST_EMPTY"


async def test_remove_item_audited(client, make_client, make_staff_user, auth_app):
    """Removing a non-last item succeeds (204) and writes one WatchlistItemRemoved row."""
    tenant, h = await _admin(client, make_client, make_staff_user)
    wl = (await client.post(f"/clients/{tenant.id}/watchlists", headers=h, json=_payload())).json()
    target = wl["items"][0]["id"]
    resp = await client.delete(
        f"/clients/{tenant.id}/watchlists/{wl['id']}/items/{target}", headers=h
    )
    assert resp.status_code == 204
